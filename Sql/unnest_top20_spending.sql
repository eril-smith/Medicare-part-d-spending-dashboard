-- File: unnest_top20_spending.sql
-- Description: Returns top 20 drugs by CAGR with total spending data unnested by year
-- Notes: Uses UNNEST for year-over-year comparisons. 
...
JOIN LATERAL (
	SELECT
    	...
    	UNNEST(ARRAY[Tot_Spndng_2018
			,Tot_Spndng_2019
			,Tot_Spndng_2020
			,Tot_Spndng_2021
			,Tot_Spndng_2022]) AS tot_spndng
) AS unnested ON TRUE
WHERE rn<= 20
