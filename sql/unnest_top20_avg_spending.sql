-- File: unnest_top20_avg_spending.sql
-- Description: Returns top 20 drugs by CAGR with average spending per dosage unit (unnested)
-- Notes: Highlights changes in price-per-unit over time
...
JOIN LATERAL (
	SELECT
    	...
    	UNNEST(ARRAY[avg_spnd_per_dsg_unt_wghtd_2018
		,avg_spnd_per_dsg_unt_wghtd_2019
		,avg_spnd_per_dsg_unt_wghtd_2020
		,avg_spnd_per_dsg_unt_wghtd_2021
		,avg_spnd_per_dsg_unt_wghtd_2022]) AS avg_spnd
) AS unnested ON TRUE
WHERE rn<= 20
